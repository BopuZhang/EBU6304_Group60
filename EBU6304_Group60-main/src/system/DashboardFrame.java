...
//========== MO Functions ==========
    ...
    private void refreshTaTable(JPanel tablePanel, List<User> taUsers, Map<String, Profile> profileMap,
                                String searchText, String selectedMajor, String selectedGrade) {
        tablePanel.removeAll();
        ...
        headerPanel.add(createHeaderLabel("Student ID"));
        headerPanel.add(createHeaderLabel("Major"));
        headerPanel.add(createHeaderLabel("Grade"));
        headerPanel.add(createHeaderLabel("Action"));
        tablePanel.add(headerPanel);

        // 筛选和搜索
        List<User> filteredTAs = new ArrayList<>();
        for (User user : taUsers) {
            Profile profile = profileMap.get(user.getEmail());
            ...
            boolean matchesMajor = selectedMajor.equals("All Majors") ||
                    (profile != null && selectedMajor.equals(profile.getMajor()));
            if (!matchesMajor) continue;

            boolean matchesGrade = selectedGrade.equals("All Grades") ||
                    (profile != null && selectedGrade.equals(profile.getGrade()));
            if (!matchesGrade) continue;

            filteredTAs.add(user);
        }
        ...
        if (filteredTAs.isEmpty()) {
            ...
            JLabel emptyLabel = new JLabel("No TAs found matching your criteria.", SwingConstants.CENTER);
            emptyLabel.setFont(new Font("Segoe UI", Font.PLAIN, 14));
            emptyLabel.setForeground(new Color(150, 150, 150));
            emptyPanel.add(emptyLabel);
            tablePanel.add(emptyPanel);
        } else {
            for (User user : filteredTAs) {
                Profile profile = profileMap.get(user.getEmail());
                ...
                rowPanel.setPreferredSize(new Dimension(0, 55));

                rowPanel.add(createCellLabel(user.getName()));

                String studentId = (profile != null && profile.getStudentId() != null && !profile.getStudentId().isEmpty())
                        ? profile.getStudentId() : "Not set";
                rowPanel.add(createCellLabel(studentId));

                String major = (profile != null && profile.getMajor() != null && !profile.getMajor().isEmpty())
                        ? profile.getMajor() : "Not set";
                rowPanel.add(createCellLabel(major));

                String grade = (profile != null && profile.getGrade() != null && !profile.getGrade().isEmpty())
                        ? profile.getGrade() : "Not set";
                rowPanel.add(createCellLabel(grade));
                ...
                viewBtn.setPreferredSize(new Dimension(100, 28));
    ...
    /**
     * Refresh TA table with search and filter
     */
